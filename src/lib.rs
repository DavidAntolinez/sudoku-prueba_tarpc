...
pub struct LogBuffers {
    pub client: Arc<Mutex<Vec<String>>>,
    pub server: Arc<Mutex<Vec<String>>>,
    pub rpc: Arc<Mutex<Vec<String>>>
}
...
impl std::io::Write for BufferWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        let mut buffer_biding = self.buffer.lock().unwrap();
        buffer_biding.push(text.clone());

        if buffer_biding.len() > 1000 {
            buffer_biding.remove(0);
        }
...
impl<'a> MakeWriter<'a> for MultiWriter {
    ...
    fn make_writer(&'a self) -> Self::Writer {
        BufferWriter {
            buffer: self.buffers.server.clone(), // default
        }
    }
    ...
    fn make_writer_for(&'a self, meta: &tracing::Metadata<'_>) -> Self::Writer {
        ...
        if target.contains("cliente") {
            BufferWriter {
                buffer: self.buffers.client.clone(),
            }
        } else if target.contains("server") {
            BufferWriter {
                buffer: self.buffers.server.clone(),
            }
        } else {
            BufferWriter {
                buffer: self.buffers.rpc.clone(),
            }
        }
    }
}
...
pub fn init_tracing(
    ...
    let ui_layer = tracing_subscriber::fmt::layer()
        .with_writer(MultiWriter { buffers })
        .with_target(true)
